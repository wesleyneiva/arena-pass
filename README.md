...
- **Convite do aluno com QR Code**: gerado ao confirmar o agendamento; validado publicamente por token, com janela de validade de 1h antes do início da aula até o fim (ex: aula das 8h às 8:59, convite vale das 7h às 8:59). Não é possível emitir convite novo para uma aula cujo horário já passou. O admin/master também visualiza os convites emitidos de cada agendamento (aluno, CPF, status) direto na tela de Agendamentos, pra saber quem vai chegar no clube.
