...
- **Agendamento**: professor escolhe quadra + data e vê a grade de horários livres; pode reservar múltiplos slots (não precisam ser contíguos) em uma única ação. Conflito de horário é bloqueado de verdade no banco (constraint de exclusão no Postgres), não só na aplicação.
- **Pagamento da taxa**: Pix (com geração real de BR Code/EMV, QR e copia-e-cola), cartão ou dinheiro — o próprio professor confirma o pagamento, ou o admin confirma manualmente.
- **Convite do aluno com QR Code**: gerado ao confirmar o agendamento; validação pública por token, com janela de tolerância em torno do horário da aula.
